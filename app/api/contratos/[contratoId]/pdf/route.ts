...
import { z } from "zod";
import { gerarPdfContratoPublico } from "@/lib/contratos/services";
import {
  contratoIdDaRota,
  erroContratoPublico,
...
export async function POST(request: NextRequest, context: ContratoRouteContext) {
  try {
    ...
    const data = await gerarPdfContratoPublico({ contratoId, provaToken: body.provaToken, acessoToken: body.acessoToken });
    return new NextResponse(new Uint8Array(data.pdf), {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="contrato-oficial-kidmais-v${data.versao.numeroVersao}.pdf"`,
