...
export async function GET(request: NextRequest) {
  ...
  try {
    const resultado = await db().query<{codigo:string;nome:string;categoria:string;valor:string}>(`
      SELECT a.codigo,a.nome,a.categoria,preco.valor::text AS valor
    ...
    return NextResponse.json({ adicionais: resultado.rows.filter(row=>ids[row.codigo]).map(row=>({
      id:ids[row.codigo],nome:row.nome,categoria:row.categoria,preco:Number(row.valor),codigo:row.codigo,
    })) }, { headers:noStore });
