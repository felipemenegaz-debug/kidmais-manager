...
const CHAVE_INICIAL = 'kidmais:perfil-empresa:provisionamento-inicial';

function chave(empresaId) {
    return `kidmais:perfil-empresa:${empresaId}`;
}

async function travarInicial(tx) {
    await tx.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [CHAVE_INICIAL]);
}
