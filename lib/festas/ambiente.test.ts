...
test('flag explícita + estrutura final independem do nome físico do banco',async()=>{
    ...
    try{
        ...
        const tx={query:async(sql:string)=>{assert(!/current_database|kidmais_016_/i.test(sql));assert(!/\b(CREATE|ALTER|INSERT|UPDATE|DELETE|DROP)\b/.test(sql));return {rows:[{assinatura:assinaturaEstruturaFesta016,nome:'kidmais_cloud_producao'}],rowCount:1};}} as DbExecutor;
