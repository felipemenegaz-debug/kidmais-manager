...
test('inventory excludes rollback and never claims applied state', async () => {
  const r = await migrations.check({}); assert.deepEqual(r.blockers, []); assert.equal(r.evidence[0].appliedState, 'unknown'); assert.equal(r.evidence[0].latest, '20260926_033_tabela_preco_publicacao.sql'); assert.ok(!r.evidence[0].migrations.some(x => x.includes('999')));
