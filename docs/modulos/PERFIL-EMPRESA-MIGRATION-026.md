...
Data: 25/09/2026. Status: executadas só na revisão local descartável `kidmais_perfil_v1_revisao` em `127.0.0.1:55432`. Staging e produção continuam sem essas migrations. O código de autorização, cadastro e a tela `/admin/configuracoes/perfil-empresa` estão na branch `review/v1-perfil-empresa`. O recurso não entra em staging nem em produção enquanto Felipe não autorizar aquele ambiente. A 028 corrige o CHECK da revisão aplicada (`motivo IS NOT NULL`) e acrescenta quem aplicou; ela não reescreve 026 nem 027. Doubles não comprovam o PostgreSQL; a revisão local, em conexões distintas, comprovou só aquele banco descartável.
...
1. Nenhuma concessão poderá ocorrer antes da implementação das proteções nos serviços de usuários. `desativarUsuarioAdministrativo` e o `atualizar` de `scripts/admin-provision.cjs` usam a trava por empresa e recusam a última administradora elegível. Staging e produção continuam sem esta migration e sem `INSERT` em `perfil_empresa_concessoes`. A revisão local descartável não altera essa pendência.
...
4. Análise estática não comprova execução ou concorrência em staging ou produção. O teste que lê os arquivos não abre conexão e não prova FK, índice parcial, forma devolvida por `pg_get_constraintdef` nem corrida entre sessões. A cobertura que recusa omitir `motivo_revogacao IS NOT NULL` só olha o texto da migration. Ela não substitui o teste futuro no PostgreSQL. A revisão local descartável de 25/09/2026 executou os checks e rejeitou motivo nulo na revisão aplicada; isso não substitui o mesmo ensaio em staging ou produção.
