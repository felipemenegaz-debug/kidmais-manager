...
export default function ClienteLixeira({clienteId,registro,onAlterado}:{clienteId:string;registro?:RegistroLixeira;onAlterado?:()=>void}) {
    ...
    return <section className={styles.integrationNotice} aria-label="Situação do cadastro">
        ...
        {etapa>0&&<div role="group" aria-label="Confirmação de alteração do cadastro">
            ...
            <label>Motivo obrigatório <textarea maxLength={500} value={motivo} disabled={busy||etapa===2} onChange={e=>setMotivo(e.target.value)}/></label>
            ...
            <button type="button" className={styles.secondaryButton} disabled={busy} onClick={()=>setEtapa(0)}>Cancelar</button>
            {item.status==='ATIVO'&&etapa===1?<button type="button" className={styles.primaryButton} disabled={motivo.trim().length<3} onClick={()=>setEtapa(2)}>Continuar para segunda confirmação</button>:<button type="button" className={styles.primaryButton} disabled={busy||motivo.trim().length<3} onClick={confirmar}>{busy?'Salvando…':item.status==='INATIVO'?'Restaurar cliente':modo==='EXCLUIR'?'Confirmar exclusão lógica':'Confirmar arquivamento'}</button>}
        </div>}</>}
    </section>;
}
