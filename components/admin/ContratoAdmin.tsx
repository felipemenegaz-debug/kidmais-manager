...
type Painel = {
    revisoesOperacionais:Array<{id:string;contrato_versao_id:string;estado:string;hold_destino_adquirido_em:string|null;status_fechamento:string;data_evento:string;data_vigente:string;horario_inicio:string;horario_fim:string;slot_alterado:boolean}>;
...
export default function ContratoAdmin() {
    ...
    const action = async (body: Record<string, unknown>):Promise<boolean> => { if (!v)
        return false; setBusy(true); setError(''); try {
 ...
 <select aria-label="Versão contratual" value={vid} onChange={e => { const n = data.versoes.find(x => x.id === e.target.value)!; setVid(n.id);setEditando(false); setNote(n.dados_fonte?.observacoesDocumentais ?? ''); setKey(''); }}>{data.versoes.map(x => <option key={x.id} value={x.id}>V{x.numero_versao} — {x.estado_edicao ?? `LEGADO / ${x.status}`}</option>)}</select>
 ...
 {revisaoOperacional && <section className={styles.card}><h2>Agenda da revisão</h2><p>{revisaoOperacional.status_fechamento==='CONFIRMADO'?(!revisaoOperacional.slot_alterado&&['EM_ELABORACAO','CONGELADA'].includes(revisaoOperacional.estado)?'RESERVA CONFIRMADA — data e horário mantidos':'RESERVA CONFIRMADA'):'DATA PROPOSTA — AINDA NÃO RESERVADA'} · data vigente: {revisaoOperacional.data_vigente}</p>{['EM_ELABORACAO','CONGELADA'].includes(revisaoOperacional.estado)&&<>{revisaoOperacional.slot_alterado&&<p>{revisaoOperacional.hold_destino_adquirido_em?'HOLD DE REMARCAÇÃO — DATA PROVISORIAMENTE PROTEGIDA DURANTE REMARCAÇÃO':revisaoOperacional.status_fechamento==='CONFIRMADO'?'DESTINO SEM HOLD — REVALIDAÇÃO OBRIGATÓRIA':'DATA PROPOSTA — AINDA NÃO RESERVADA'} · destino: {revisaoOperacional.data_evento} · {revisaoOperacional.horario_inicio.slice(0,5)}–{revisaoOperacional.horario_fim.slice(0,5)}</p>}<button disabled={busy} onClick={()=>action({acao:'revalidar_destino',revisao:v.revisao})}>Revalidar destino</button><button disabled={busy} onClick={()=>{const m=window.prompt('Motivo do cancelamento da revisão. A versão vigente será preservada.');if(m&&m.trim().length>=3)action({acao:'cancelar_revisao',revisao:v.revisao,motivo:m});}}>Cancelar revisão</button></>}</section>}
 <section id="alteracoes" tabIndex={-1} aria-label="Alterações da contratação" className={styles.anchor}><h2>Alterações da contratação</h2>{v.status === 'ASSINADA' && (!data.fluxo || data.fluxo.versao_vigente_id===v.id) && <section><h3>Iniciar uma alteração</h3><p>Prepare alterações de convidados, pacote, adicionais, buffet, data e condição comercial em uma nova versão. Você pode iniciar por necessidade da Kidmais.</p><select value={tipo} onChange={e => setTipo(e.target.value)}><option value="NOVA_VERSAO">Nova versão</option><option value="RETIFICACAO">Retificação documental</option><option value="ADITIVO" disabled>Aditivo — template específico pendente</option></select><label>Motivo <input value={motivo} maxLength={500} onChange={e => setMotivo(e.target.value)}/></label><button disabled={busy || motivo.trim().length < 3} onClick={() => action({ acao: 'nova_versao', tipo, motivo, chaveCriacao: crypto.randomUUID() })}>Iniciar elaboração</button></section>}{v.estado_edicao==='EM_ELABORACAO'&&<button disabled={busy} onClick={()=>setEditando(!editando)}>{editando?'Fechar edição':'Editar dados desta revisão'}</button>}{editando && <EdicaoFesta key={vid} versaoId={vid} revisao={v.revisao} serverError={error} onSave={action} onClose={()=>setEditando(false)}/>}<h3>Alterações desta revisão</h3>{v.alteracoes?.campos?.length ? <div className={styles.tableWrap}><table><thead><tr><th>Campo</th><th>Antes</th><th>Depois</th></tr></thead><tbody>{v.alteracoes.campos.map(d=><tr key={d.campo}><td>{rotuloCampo(d.campo)}</td><td>{valorDiferenca(d.antes,d.campo)}</td><td>{valorDiferenca(d.depois,d.campo)}</td></tr>)}</tbody></table></div>:<p>Nenhuma diferença registrada nesta elaboração.</p>}</section>
