...
export default function ContratoAdmin() {
    ...
    async function criarRevisao(){
        ...
        if(await action({acao:'nova_versao',tipo,motivo:motivo.trim(),chaveCriacao:pedidoRevisao.current.chave})){pedidoRevisao.current=null;setEditando(false);window.location.hash='alteracoes';}
    }
 ...
 <section id="alteracoes" tabIndex={-1} aria-label="Alterações da contratação" className={styles.anchor}><h2>Alterações da contratação</h2>{podeRevisar && <section><h3>Criar revisão / retificação</h3><p>Prepare alterações de convidados, pacote, adicionais, buffet, data e condição comercial em uma nova versão. Você pode iniciar por necessidade da Kidmais.</p><select aria-label="Tipo da revisão" value={tipo} onChange={e => setTipo(e.target.value)}><option value="NOVA_VERSAO">Nova versão</option><option value="RETIFICACAO">Retificação documental</option><option value="ADITIVO" disabled>Aditivo — template específico pendente</option></select><label>Motivo <input value={motivo} maxLength={500} onChange={e => setMotivo(e.target.value)}/></label>{preparacaoAtiva?<button disabled={busy} onClick={()=>{setVid(preparacao.id);setNote(preparacao.dados_fonte?.observacoesDocumentais??'');setEditando(false);}}>Abrir revisão em andamento — V{preparacao.numero_versao}</button>:<button disabled={busy || motivo.trim().length < 3} onClick={criarRevisao}>Criar revisão / retificação</button>}</section>}{v.estado_edicao==='EM_ELABORACAO'&&<button disabled={busy} onClick={()=>setEditando(!editando)}>{editando?'Fechar edição':'Editar dados desta revisão'}</button>}{editando && <EdicaoFesta key={vid} versaoId={vid} revisao={v.revisao} serverError={error} onSave={action} onClose={()=>setEditando(false)}/>}<h3>Alterações desta revisão</h3>{origem&&<p>Versão anterior: V{origem.numero_versao} → Nova versão: V{v.numero_versao} · Status: {v.estado_edicao} · Motivo: {v.motivo_nova_versao} · Criada em: {new Date(v.criado_em).toLocaleString('pt-BR')}</p>}{analise&&<><p>{analise.natureza==='MATERIAL'?'Alteração material':analise.natureza==='DOCUMENTAL'?'Alteração documental / não material':'Nenhuma alteração de conteúdo'}. Esta revisão exige nova assinatura da Kidmais e do cliente para entrar em vigor, inclusive em correções documentais.</p>{analise.alteraAgenda&&<p>A data e o horário vigentes continuam ocupados até a formalização desta revisão. O destino será revalidado; conflitos impedem a troca.</p>}{analise.impactoFinanceiro&&<p>Há impacto financeiro. Recebimentos anteriores são preservados. Após formalização, confira as pendências e ajuste as obrigações futuras pelo fluxo financeiro existente; não há quitação ou estorno automático.</p>}</>}{(analise?.campos ?? v.alteracoes?.campos)?.length ? <div className={styles.tableWrap}><table><thead><tr><th>Campo</th><th>Antes</th><th>Depois</th></tr></thead><tbody>{(analise?.campos ?? v.alteracoes?.campos ?? []).map(d=><tr key={d.campo}><td>{rotuloCampo(d.campo)}</td><td>{valorDiferenca(d.antes,d.campo)}</td><td>{valorDiferenca(d.depois,d.campo)}</td></tr>)}</tbody></table></div>:<p>Nenhuma diferença registrada nesta elaboração.</p>}</section>
